...
Gamma = np.diag([2.5**2, 2.5**2]) # was: 0.8, 0.8
mean_theta = np.array([0.1, 0.2])
...
posteriorlogdensity = lambda theta: misfit(theta) + 0.5*(theta-mean_theta) @ (np.linalg.inv(Gamma) @ (theta-mean_theta))

N_contours =150
theta1s = np.linspace(-10, 5, N_contours)
